...
console.log("[LN][api] source-loaded");
...
function requiresAuth(path) {
  ...
  if (normalized === "/v1/csrf" || normalized === "/v1/bootstrap") return false;
  if (normalized.startsWith("/v1/auth/")) return false;
  return true;
}

function buildAuthRequiredError({ requestId = null, details = null, payload = null } = {}) {
...
export async function apiFetch(path, options = {}) {
  ...
  const attemptFetch = async (retried) => {
    try {
      if (typeof window !== "undefined" && !window.__LN_AUTH_DEBUG_LOGGED__) {
        window.__LN_AUTH_DEBUG_LOGGED__ = true;
