import {
  ...
  setDeviceName,
  getDeviceName,
} from "./app.api.202603080443-f4d1607.js";
import { getAppState as getAppStateInternal, setAppState } from "./app.state.202603080443-f4d1607.js";
import { qs, qsa, el, clear, setText, formatMinutes, formatPct, applyI18n, getDictValue } from "./app.ui.202603080443-f4d1607.js";
...
export const BUILD_ID = "202603080443-f4d1607";
...
let currentSessionPhases = null;

function populateTodayScreen(res, stress) {
  const saved = JSON.parse(localStorage.getItem("livenew_today") || "{}");
  const sessions = res?.sessions || saved?.contract?.sessions || [];
  const meals = res?.meals || saved?.contract?.meals || [];
  const completedSessions = saved?.completedSessions || {};
  ...
  if (allDone) {
    ...
    if (timeLabel) timeLabel.textContent = nextSession.time || "";
    if (titleEl) titleEl.textContent = nextSession.title || "";
    if (descEl) descEl.textContent = nextSession.description || "";
    if (startBtn) {
      startBtn.onclick = () => {
        currentSessionIndex = nextSession.index;
        currentSessionPhases = nextSession.phases || [];
        const sessionStep = qs('[data-step="move"]') || qs('[data-step="reset"]') || qs('[data-step="session"]');
        if (sessionStep) {
          const stepName = sessionStep.dataset.step;
          showStep(stepName);
          if (typeof startGuidedExperience === "function") {
            startGuidedExperience(currentSessionPhases, {
              instructionEl: sessionStep.querySelector(".reset-instruction")?.id || "reset-instruction",
              timerEl: sessionStep.querySelector(".reset-timer")?.id || "reset-timer",
              skipBtn: sessionStep.querySelector('[id$="-skip"]')?.id,
              progressEl: sessionStep.querySelector(".phase-progress")?.id,
              exitBtn: sessionStep.querySelector(".exit-session")?.id,
              onComplete: async () => {
                const plan = JSON.parse(localStorage.getItem("livenew_today") || "{}");
                if (!plan.completedSessions) plan.completedSessions = {};
                plan.completedSessions[nextSession.index] = true;
                localStorage.setItem("livenew_today", JSON.stringify(plan));
                try {
                  await apiPost("/v1/move/complete", { dateISO: currentDateISO });
                } catch {}
                populateTodayScreen(res, stress);
                showStep("today");
              },
            });
          }
    ...
    if (afterNext && nextUp) {
      ...
      if (nextText) nextText.textContent = `Up next: ${afterNext.title} · ${afterNext.time}`;
    } else if (nextUp) {
  ...
  if (completedList) {
    ...
    if (doneSessions.length > 0 && !allDone) {
      completedList.innerHTML = doneSessions.map((s) => `<div class="completed-item">✓ ${s.title}</div>`).join("");
      completedList.classList.remove("hidden");
    } else {
      completedList.innerHTML = "";
      if (!allDone) completedList.classList.add("hidden");
    }
  }

  const timelineContainer = qs("#today-timeline-sessions");
  if (timelineContainer) {
    timelineContainer.innerHTML = sessionList.map((s) =>
      `<div class="timeline-slot">` +
        `<div class="timeline-time">${s.time || ""}</div>` +
        `<div class="timeline-content">` +
          `<div class="timeline-title">${s.title || ""}</div>` +
          `<div class="timeline-status">${s.done ? "Done ✓" : ""}</div>` +
        `</div>` +
      `</div>`
    ).join("");
  }

  const mealsContainer = qs("#today-meals");
  if (mealsContainer) {
    mealsContainer.innerHTML = meals.map((m) =>
      `<div class="today-nutrition-bar">` +
        `<span class="nutrition-label">${m.time || ""}</span>` +
...
async function loadControllers() {
  if (!controllersModule) {
    controllersModule = import("./controllers.202603080443-f4d1607.js");
  }
...
function initDay({ initialDateISO } = {}) {
  ...
  const routineTextarea = document.getElementById("onboard-routine");
  const goalTextarea = document.getElementById("onboard-goal");
  const routineNextBtn = document.getElementById("routine-next");
  const goalNextBtn = document.getElementById("goal-next");

  if (routineTextarea) {
    routineTextarea.addEventListener("input", () => {
      onboardRoutine = routineTextarea.value.trim();
      if (routineNextBtn) routineNextBtn.disabled = onboardRoutine.length < 10;
    });
  }

  if (routineNextBtn) {
    routineNextBtn.addEventListener("click", () => {
      if (onboardRoutine.length < 10) return;
  ...
  if (goalTextarea) {
    goalTextarea.addEventListener("input", () => {
      onboardGoal = goalTextarea.value.trim();
      if (goalNextBtn) goalNextBtn.disabled = onboardGoal.length < 5;
    });
  }

  if (goalNextBtn) {
    goalNextBtn.addEventListener("click", async () => {
      if (onboardGoal.length < 5) return;
      goalNextBtn.textContent = "Setting up...";
      goalNextBtn.disabled = true;

      const profile = { routine: onboardRoutine, goal: onboardGoal };

      try {
        await apiPost("/v1/onboard/complete", { profile });
      } catch (err) {
        console.error("[ONBOARD_SAVE]", err);
      }
  ...
  document.querySelectorAll(".stress-tap-btn").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      const profile = JSON.parse(localStorage.getItem("livenew_profile") || "{}");
      ...
      try {
        ...
        saveTodayPlan(currentDateISO, res, stressValue);
        populateTodayScreen(res, stressValue);
        showStep("today");
        setTrialStarted();
        void requestNotificationPermission();
        scheduleNotifications("normal");
      } catch (err) {
        clearInterval(loadingInterval);
        showStep("stress-tap");
        if (isAuthRequiredError?.(err)) {
  ...
  try {
    ...
    if (cached) {
      ...
      if (p && p.routine && p.goal) hasProfile = true;
    }
  } catch {}
